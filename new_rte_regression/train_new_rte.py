...
# ==================================================================    
    ...
    if args.gpu == 0:
        ...
        if not args.evaluate and val_acc > args.best_acc:
            ...
            if args.exclude_nei:
                path = os.path.join(args.checkpoints_dir, "best_ckpt_nei_excluded.pth")
            else:
...
def calculate_metric(args, tanh_score, doc_labels_cat):
    assert len(tanh_score) == len(doc_labels_cat)
    sf, st = np.quantile(tanh_score, 0.5-args.nei_scale*0.5), np.quantile(tanh_score, 0.5+args.nei_scale*0.5)
    is_correct = []
    pred = []

    # name2label = {'False': -1.0, 'None': 0.0, 'True': 1.0}
    for l, p in zip(doc_labels_cat, tanh_score):
        if p < sf:
            p = -1 # Refuted
        elif p >= sf and p <st:
            p = 0 # NEI
