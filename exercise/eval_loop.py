...
if __name__ == "__main__":
    ...
    parser.add_argument("--claim",
                        default="카리브해의 가장 깨끗한 섬 국가들은 열대성 기후를 가지고 있고 건기와 우기로 나뉜다.",
                        type=str,
    ...
    for i in tqdm(claims_to_test):
        ...
        args.label = test_tbl[i]['True_False']
        is_correct, rte_score = main(args)
        test_tbl[i]['is_correct'] = is_correct
        test_tbl[i]['rte_score'] = rte_score
    
    with open('test_accuracy.pkl', 'wb')as f:
        pickle.dump(test_tbl, f)